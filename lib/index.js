...
module.exports = function (app) {
	...
	app.use('*', function (req, res, next) {
		...
		for (var i = 0; i < objRedirectJSON['redirects'].length; i++) {
			...
			var jsonStatusCode = parseInt(objRedirectJSON['redirects'][i].httpStatusCode);
			var jsonMatchType = objRedirectJSON['redirects'][i].matchType.toString().trim();
			...
			if (jsonMatchType == 'absolute') {
				...
				var jsonregex = objRedirectJSON['redirects'][i].regex.toString().trim();
				var jsonRegexFlags = objRedirectJSON['redirects'][i].regexFlags.toString().trim();
